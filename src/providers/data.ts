...
const baseDataProvider =
  supabaseClient !== null ? supabaseDataProvider(supabaseClient) : null;

const fallbackDataProvider: DataProvider = {
  ...
  ) => ({ data: params.variables as unknown as TData }),
...
const activeBaseDataProvider: DataProvider =
  baseDataProvider ?? fallbackDataProvider;
...
const isAdminPlanResource = (resource: string) =>
  ["admin-plans", "tiers"].includes(resource);
...
const isAdminRoute = () =>
  ...
  isAdminContext(window.location.pathname, window.location.hostname);
...
const normalizeExpensePayload = (values: Record<string, unknown>) => ({
  ...
  currency: values.currency || "ZAR",
  payment_method:
    values.payment_method || values.paymentMethod || "Bank Transfer",
  date: values.date,
...
const isQuoteRecord = (record: Record<string, unknown>) =>
...
const normalizePlanPayload = (values: Record<string, unknown>) => ({
  ...
  billing_cycle:
    String(values.billing_cycle ?? values.billingCycle ?? "monthly")
      .trim()
      .toLowerCase() === "yearly"
      ? "yearly"
      : "monthly",
...
export const dataProvider: DataProvider = {
    ...
    try {
      if (useApiForResource(resource, originalResource)) {
        ...
        if (resource === "invoice_items") {
          const invoiceId = params.filters?.find(
            (filter) => "field" in filter && filter.field === "invoice_id",
          );
          const invoiceIdValue =
            invoiceId && "value" in invoiceId ? invoiceId.value : "";
          return await apiRequest<{ data: unknown[]; total: number }>(
            `/${resource}?invoice_id=${encodeURIComponent(
      ...
      return await activeBaseDataProvider.getList({ ...params, resource });
    } catch (error) {
  ...
  getMany: async (params) => {
    ...
    try {
      ...
      return await (activeBaseDataProvider.getMany?.({
        ...params,
        resource,
      }) ?? { data: [] as BaseRecord[] });
    } catch (error) {
  ...
  create: async (params) => {
    ...
    try {
      if (useApiForResource(resource, originalResource)) {
        if (isAdminPlanResource(originalResource)) {
          return await apiRequest<{ data: unknown }>("/plans", {
            method: "POST",
            body: JSON.stringify(
              normalizePlanPayload(params.variables as Record<string, unknown>),
            ),
          });
        }
        if (isAdminOnlyResource(originalResource) && isAdminRoute()) {
          return await apiRequest<{ data: unknown }>(
            getAdminResourcePath(originalResource),
            {
              method: "POST",
      ...
      return await activeBaseDataProvider.create({ ...params, resource });
    } catch (error) {
  ...
  update: async (params) => {
    ...
    try {
      ...
      return await activeBaseDataProvider.update({ ...params, resource });
    } catch (error) {
  ...
  deleteOne: async (params) => {
    ...
    try {
      if (useApiForResource(resource, originalResource)) {
        const path =
          isAdminPlanResource(originalResource) ||
          (isAdminOnlyResource(originalResource) && isAdminRoute())
            ? getAdminResourcePath(originalResource, params.id)
            : `/${resource}/${params.id}`;
      ...
      return await activeBaseDataProvider.deleteOne({ ...params, resource });
    } catch (error) {
      console.error("[DATA PROVIDER] deleteOne error:", resource, error);
      throw toHttpError(error);
    }
  },
  getApiUrl: () => activeBaseDataProvider.getApiUrl(),
} as DataProvider;
