  ...
  if (isLocalAuthEnabled()) {
    try {
      const session =
        mode === "signup"
          ? await signUpLocally(
              fullName || email || "Local User",
              email || "",
              password || "",
              "user",
            )
          : await signInLocally(email || "", password || "", "user");
...
export const authProvider: AuthProvider = {
  login: async ({ email, password }) => {
    ...
    const returnTo = selectedPlan
      ? canStartTrialWithoutCard(selectedPlan)
        ? "/plans"
        : "/auth/card-setup"
      : "/dashboard";
    if (isLocalAuthEnabled()) {
      return beginCustomerAuthFlow(
        "login",
        email,
        undefined,
        returnTo,
    ...
    return beginCustomerAuthFlow("login", email, undefined, returnTo, password);
  },
  ...
  register: async ({ email, name, password }) => {
    ...
    if (isLocalAuthEnabled()) {
      try {
        return await beginCustomerAuthFlow(
          "signup",
          email,
          name,
          "/dashboard",
    ...
    try {
      ...
      return {
        ...
        redirectTo: `/verify-email?${new URLSearchParams({
          email,
          ...(getSelectedPlanCheckout()?.id
            ? { plan: getSelectedPlanCheckout()!.id }
            : {}),
        }).toString()}`,
      };
    } catch (error) {
...
export const resendConfirmationEmail = async () => {
  return {
    success: false,
    error:
      "Email confirmation is handled by Auth0. Enable Auth0 email verification for the app and use its verification email flow.",
  };
};
